...
class Database {
    ...
    /**
     * Returns list of all Airports supported.
     *
     **/
    static getAirports(callback) {
      let airportPath = "/airport";
      firebase.database().ref(airportPath).orderByChild('order').once('value', (snapshot) => {
        var airports = [];
        if (snapshot.val()) {
            Object.keys(snapshot.val()).forEach(function(airportId) {
                let airport = new Airport(airportId, snapshot.val()[airportId]);
    ...
    /**
     * Returns list of all Terminals supported at input airport.
     *
     **/
    static getTerminals(airportId, callback) {
      let terminalsPath = "/terminal/" + airportId;
      firebase.database().ref(terminalsPath).orderByChild('order').once('value', (snapshot) => {
        var terminals = [];
        if (snapshot.val()) {
          Object.keys(snapshot.val()).forEach(function(terminalId) {
            let terminal = new Terminal(terminalId, snapshot.val()[terminalId]);
    ...
    /**
     * Returns list of all Gates at the input airport and terminal.
     *
     **/
    static getGates(airportId, terminalId, callback) {
      let gatesPath = "/gate/" + airportId + "_" + terminalId;
      firebase.database().ref(gatesPath).orderByChild('order').once('value', (snapshot) => {
        var gates = [];
        if (snapshot.val()) {
          Object.keys(snapshot.val()).forEach(function(gateId) {
            let gate = new Gate(gateId, snapshot.val()[gateId]);
    ...
    /**
     * Returns list of all Restaurants at the input airport and terminal.
     *
     **/
    static getRestaurants(airportId, terminalId, callback) {
      let restaurantsPath = "/restaurant/" + airportId + "_" + terminalId;
      firebase.database().ref(restaurantsPath).once('value', (snapshot) => {
        var restaurants = [];
        if (snapshot.val()) {
          Object.keys(snapshot.val()).forEach(function(restaurantId) {
            let restaurant = new Restaurant(restaurantId, snapshot.val()[restaurantId]);
